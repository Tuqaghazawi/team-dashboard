...
Configured for Render over HTTPS with PostgreSQL. `render.yaml` is a blueprint —
point Render at this repository (New > Blueprint) and it provisions the web
service and the database. `build.sh` installs, collects static files and
migrates.

Set `OPENAI_API_KEY` in the Render dashboard; it is deliberately not in the
blueprint. Without it the AI features report "unavailable" and the rest of the
pathway still works. `DJANGO_SECRET_KEY` is generated by Render.

Everything hardening-related is conditional on `DEBUG` being off, so local
development is unaffected: HTTPS redirect, secure session and CSRF cookies,
`nosniff`, `X-Frame-Options: DENY`. HSTS starts at 0 and should only be raised
once the deployment is known good — a cached HSTS header cannot be withdrawn
early.
